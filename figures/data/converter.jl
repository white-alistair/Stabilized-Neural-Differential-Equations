...
begin
    ...
    E_rel_err_snde = zeros(N, length(times_multi))
    E_rel_err_node = zeros(N, length(times_multi))
    E_rel_err_anode = zeros(N, length(times_multi))
    E_rel_err_sanode = zeros(N, length(times_multi))
    ...
    Threads.@threads for i = 1:N
        ...
        g = @closure (u, t) ->
            constraints(u, t, system, Val(exp_version)) .-
            constraints(u0, t0, system, Val(exp_version))
        E0 = abs(constraints(u0, t0, system, Val(exp_version))[1])
        ...
        E_rel_err_node[i, 1:size(predicted_traj)[2]] .=
            vec(mapslices(u -> abs(g(u, nothing)[1]) / E0, predicted_traj, dims = 1))
...
data_path = "figures/data/converter.jld2"
JLD2.save_object(
    data_path,
    (
        times_single,
